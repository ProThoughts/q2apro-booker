...
*/
	...
	class booker_page_requestlist
	{
		...
		{	
			return array(
				array(
					'title' => 'booker Page requestlist', // title of page
					'request' => 'requestlist', // request name
		...
		function process_request($request)
		{
			...
			qa_set_template('booker requestlist');
			...
			// page title
			$qa_content['title'] = qa_lang('booker_lang/all_requests');
			...
			{
				$doalphabetic = true;
				$count = booker_get_contractorcount(); // *** ?
			...
			$output .= '
					<div class="q2apro_usersearch_box">
						...
						<span id="q2apro_usersearchlabel">'.qa_lang('booker_lang/reli_search_label').':</span> 
						
						<input id="q2apro_usersearch" name="q2apro_usersearch" type="text" placeholder="'.qa_lang('booker_lang/reli_search_placeholder').'" value="'.$searchstring.'" autofocus /> 
			...
			<div class="" style="float:right;">
				<a href="'.qa_path('requestcreate').'" class="defaultbutton senddatabtn">
					'.qa_lang('booker_lang/ownrequest_create_btn').'
				</a>
			</div>
			';
			...
			// get all requests
			$allrequests = booker_get_all_requests();

			$metaOutput = '';
			$metacount = count($allrequests);
			...
			{
				$clirequests .= booker_requests_to_table($allrequests, $this->urltoroot);
			} // end count $allrequests
			else 
			{
				$output .= '
				<p class="qa-error">
					...
					$(document).ready(function()
					{
						...
						*/
						...
						$('#locations').on('change keyup', function() 
						{
							...
							{
								// show all
								$('#q2apro_ajaxsearch_results #requeststable tr').show();
							}
							else
							{
								$('#q2apro_ajaxsearch_results #requeststable tr').each( function()
								{
			...
			<style type="text/css">
				...
				@media only screen and (max-width:480px) {
					...
					#requeststable tr td:nth-child(1) {
						min-height:120px;
						border-bottom:0;
					}
					#requeststable tr td:nth-child(1), 
					#requeststable tr td:nth-child(2) {
						width:100%;
						display:block;
					}
					.requesttime {
